...
"""Proximal policy optimization training — **arcdrone fork**.

This file is a local copy of ``brax.training.agents.ppo.train`` with the
following modification for VRAM-efficient vision RL:

**CNN feature caching inside the rollout scan**

Instead of accumulating full pixel tensors across rollout scan iterations
and then running the CNN during every SGD minibatch step, this fork:

1. Runs the frozen CNN *once* per scan iteration on the current batch of
   pixels (``extract_cnn_features``).
2. Replaces pixel observations in the ``Transition`` with compact CNN
   feature vectors before they are accumulated (``compact_obs``).
3. The ``_policy_apply`` dual-path in ``networks.py`` detects the
   ``cnn_feats`` key and skips the CNN entirely during loss computation.

Peak pixel memory is bounded to a single scan iteration
(``num_envs × unroll_length × H × W × C``), and the data fed to SGD is
~500× smaller per observation sample.
...
# arcdrone: CNN feature-caching helpers from the local fork of networks.py
from arcdrone.vision_landing_rl.training.networks import (
...
def _remove_pixels(
  ...
  """Removes pixel observations (and cached CNN features) from the observation dict."""
  if not isinstance(obs, Mapping):
    return obs
  return {k: v for k, v in obs.items()
          if not k.startswith('pixels/') and k != 'cnn_feats'}
...
def train(
  ...
  def training_step(
    ...
    def f(carry, unused_t):
      ...
      # ── CNN feature caching (arcdrone modification) ─────────────────
      # When frozen_cnn=True: run the CNN once per scan iteration and
      # replace pixel tensors with compact feature vectors so that the
      # full pixel buffer never accumulates across scan iterations.
      if frozen_cnn:
        cnn_feats_obs = extract_cnn_features(
            training_state.params.policy[0],
            data.observation,
        )
        cnn_feats_next = extract_cnn_features(
            training_state.params.policy[0],
            data.next_observation,
        )
        data = types.Transition(
            observation=compact_obs(data.observation, cnn_feats_obs),
            action=data.action,
            reward=data.reward,
            discount=data.discount,
            next_observation=compact_obs(data.next_observation, cnn_feats_next),
            extras=data.extras,
        )
      # ── end CNN feature caching ─────────────────────────────────────
