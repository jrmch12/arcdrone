...
def train(
    ...
    *,
    ...
    # Use jit+vmap instead of pmap for the reset.  Madrona's custom JAX
    # primitives (BatchRenderer) have vmap batching rules but the pmap
    # compilation path can corrupt the CUDA context when combined with
    # StudentWrapper.  With a single GPU this is semantically equivalent.
    ...
    # ========================= jit (instead of pmap) =========================
    # Same rationale as for the reset: jit+vmap avoids the pmap compilation
    # path that is incompatible with Madrona's CUDA custom calls.

    il_epoch_jit = jax.jit(jax.vmap(il_epoch), donate_argnums=(0, 1))
    ...
    evaluator = acting.Evaluator(
        ...
        # num_eval_envs MUST equal num_envs: the BatchRenderer / MadronaWrapper
        # was initialized for exactly num_envs worlds; the Evaluator resets the
        # env with num_eval_envs keys, which would fail if they differ.
