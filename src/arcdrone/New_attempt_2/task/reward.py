...
def _check_episode_events_impl(self, state):
    ...
    landing_radius = float(getattr(self.cfg, "landing_radius", 0.4))
    in_landing_cylinder = xy_radius <= landing_radius
...
def _get_reward_impl(self, state, action):
    ...
    landing_radius = float(getattr(self.cfg, "landing_radius", 0.4))
    in_landing_cylinder = jp.linalg.norm(current_pos[0:2]) <= landing_radius
